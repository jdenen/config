...
** Usage
   ...
   Basically, I just =tangle= this README file and my machine will be set up.

   #+BEGIN_SRC sh :results silent :eval no
     C-c C-v f README.org
   #+END_SRC
** Source
*** .bashrc
**** Functions
...
**** Aliases
     ...
     shell commands. This =freq= command does just that. It's very useful to improving efficiency.
...
**** Tangle
     #+BEGIN_SRC sh :tangle foobar :export none :noweb yes
...
*** .gitconfig
    ...
    #+BEGIN_SRC conf :results silent
      ...
      [alias]
        ...
        logg = log --graph
        update-tools = !cd \"/Users/johnson/.cas-git-tools\" && git fetch && git reset --hard origin/master
